/// Main module file for file managing 
/// Can be used to read/write to build tool config files
extern crate xml;
extern crate dirs;
...
fn get_env_var_config_file(env_var_key: &str) -> SimpleResult<PathBuf> {
    ...
    match env_var {
        Ok(env_var_value) => {
            ...
            match path_to_settings.exists() { // ! TODO This should be mutualized in an utils function
                true => Ok(path_to_settings),
                false => Err(SimpleError::new(format!("Could not find the following configuration file {} ", path_to_settings.to_str().unwrap())))
            }
        },
